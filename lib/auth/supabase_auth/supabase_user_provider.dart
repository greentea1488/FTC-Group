...
class FTCGroup2SupabaseUser extends BaseAuthUser {
  FTCGroup2SupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> fTCGroup2SupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = FTCGroup2SupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
